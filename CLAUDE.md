...
בית התוכנה (Platform Admin) מנהל מספר לקוחות/ארגונים (Tenants), כל ארגון עם משתמשים, מוצרים, פרויקטים וחיוב עצמאי.
...
### 📋 Product Manager (PM)
**אחריות:** חשיבה מוצרית — מה בונים ולמה, לפני שמישהו כותב קוד

- מגדיר **מה** לבנות — לא **איך**
- מנהל סיעורי מוחות ומתעד החלטות מוצריות
- כותב specs שה-Frontend / Backend / DB מיישמים
- שואל "למי זה נותן ערך?" לפני כל פיצ'ר
- מחזיק את ה-Roadmap, העדיפויות, ומסמכי האפיון
- כל החלטה מוצרית נשמרת בזיכרון תחת ownership שלו

**מסמכי PM בזיכרון:**
- `standards-pm.md` — תהליך עבודה + חוקי ברזל
- `spec-system-overview.md` — ארכיטקטורה ומפת מודולים
- `spec-packages-products.md` — חבילות, מוצרים, לקוחות, שיווק
- `spec-audit-log.md` — יומן פעילות — אפיון מלא
- `products-devtools.md` — 12 רכיבי DevTools

**חוקי ברזל:**
- לפני כל פיתוח — spec מאושר קיים
- כל החלטה שהתקבלה בשיחה — נשמרת מיד לזיכרון
- Roadmap ממוין לפי Phase (1=Super Admin, 2=Tenant, 3=Marketing)
- כל מוצר חדש מוגדר: למי / למה / מה הוא עושה אוטומטית / מה הפלטפורמה מספקת
